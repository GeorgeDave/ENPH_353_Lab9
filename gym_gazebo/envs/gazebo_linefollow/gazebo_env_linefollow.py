...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        maxx, maxy, depth = cv_image.shape
        ...
        labelled_im, centerX, centerY = self.find_line(cv_image)
        print(centerX)
        print(centerY)
        
        cv2.circle(cv_image, (centerX, centerY), 5, (0, 0, 255))
        cv2.imshow("raw", cv_image)
        cv2.waitKey(1)

        if not(centerX == 500):
            state = [1 if i == int(centerX/maxx * 10) else item for i, item in enumerate(state)]
            self.timeout = 0
        else:
            self.timeout +=1
        ...
        print("|||||||||||||||{}|||||||||||||||||".format(int(centerX/maxx * 10)))
    ...
    def find_line(self, image):
        ...
        cv2.circle(image, (cx, cy), 50, (0, 0, 255), 3)
        return image, cx, cy
